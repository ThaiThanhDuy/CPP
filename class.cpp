...
public:
  Animal(const std::string &n, int a, const std::string &b)
  ...
  virtual void displayInfo() const {
    std::cout << "Tên: " << name << ", Tuổi: " << age
              << ", Giống loài: " << breed << std::endl;
  }
};
...
public:
  ...
  void displayInfo() const override {
    std::cout << "Đây là một con chó tên " << getName() << ", " << getAge()
              << " tuổi, giống " << getBreed() << "." << std::endl;
  }
};
...
public:
  // Constructor của Poodle, gọi constructor của Dog thông qua danh sách khởi
  // tạo thành viên
  Poodle(const std::string &n, int a, bool mini)
      : Dog(n, a, "Poodle"), isMiniature(mini) {}

  // Ghi đè phương thức makeSound để phù hợp với tiếng sủa của Poodle (ví dụ)
  void makeSound() const override { std::cout << "Yip! Yip!" << std::endl; }

  // Phương thức riêng của Poodle
  void doFancyTricks() const {
    std::cout << getName() << " đang thực hiện các trò diễn đẹp mắt!"
              << std::endl;
  }
  ...
  void displayInfo() const override {
    ...
    std::cout << (isMiniature ? "Đây là Poodle Miniature."
                              : "Đây là Poodle tiêu chuẩn.")
              << std::endl;
  }
};
